...
print("You Will Have Only 1 Chance To Answer So, Answers Wisely\n".center(170," "))
for j in round:
    if j==list1:
        print("<<<ROUND {} (((Questions Based On Astronomy)))>>>\n".format(r).center(170, " "))
        print(" Negative Marking = {}    Plus Marking = +{}".format(-neg,plus).center(170, " "))
    ...
    for i in j:
        ...
        if ans==(set[i].upper()) or ans==(set[i].lower()) or ans==(set[i]) or ans==(set[i][0].upper()) or ans==(set[i][0].lower()):
            ...
            if r == 5 and n == 5:
                ...
                result="Not Qualified"
                while True:
                    print("You not qualified ROUND {} , You want to continue to next ROUND {} ?".format(r,r+1))
                ...
                result="  Qualified  "
                while True:
                    print("You qualified ROUND {} , You want to continue to ROUND {}".format(r,r+1))
                    exit = input("Yes or No ? ")
                    if exit == "no" or exit == "N" or exit == "No" or exit == "n" or exit=="NO":
                        print("GAME OVER".center(170, " "))
                        print("Thank you for playing , Better try next time".center(170, " "))
            ...
            if r==5 and n==5:
                ...
                if Score<(total_score//2):
                    result="Not Qualified"
                    while True:
                        print("You not qualified ROUND {} ,Do You want to continue to ROUND {} ?".format(r,r+1))
                        exit=input("Yes or No ? ")
                        if exit=="no" or exit=="N" or exit=="No" or exit=="n":
                            print("GAME OVER".center(170," "))
                            print("Thank you for playing , Better try next time (-_-)".center(170," "))
                            quit()
                        elif exit=="Yes" or exit=="Y" or exit=="y" or exit=="yes":
                            break
                        else:
                            print("\nEnter a Valid Input")
                else:
...
if Score>=70:
    print("OVERALL SCORE = {} :: SCORE REQUIRED TO WIN = 70".format(Score).center(170," "))
    print("\n")
